<?php
	...
	class db {
		...
		function db($dbuser, $dbpassword, $dbname, $dbhost)
		{
			...
			if ( ! $this->dbh )
			{
				$this->dbh = @mysql_connect($dbhost,EZSQL_DB_USER,$dbpassword);
		...
		function select($D=EZSQL_DB_NAME)
		{
			if ( !@mysql_select_db($D,$this->dbh))
			{
				$this->print_error("<ol><b>Error selecting database <u>$D</u>!</b><li>Are you sure it exists?<li>Are you sure there is a valid database connection?</ol>");
                return false;
			}
            return true;
		}
		...
		function get_var($query=null,$x=0,$y=0)
		{
			...
			// Log how the function was called
			$this->func_call = "\$D->get_var(\"$query\",$x,$y)";
		...
		function get_row($query=null,$output=OBJECT,$y=0)
		{
			...
			// Log how the function was called
			$this->func_call = "\$D->get_row(\"$query\",$output,$y)";
			...
			{
				$this->print_error(" \$D->get_row(string query, output type, int offset) -- Output type must be one of: OBJECT, ARRAY_A, ARRAY_N");
			}
		...
		function get_results($query=null, $output = OBJECT)
		{
			...
			// Log how the function was called
			$this->func_call = "\$D->get_results(\"$query\", $output)";
        ...
        function insert($table, $v, $bflds)
        {
			...
			if($bflds)
				$qry = "INSERT INTO ".$table.$flds."VALUES".$values;
			else
    ...
    $D = new db(EZSQL_DB_USER, EZSQL_DB_PASSWORD, EZSQL_DB_NAME, EZSQL_DB_HOST);
